...
export default function Workspace({
  ...
  const assignedNames = useMemo(() => {
    const s = new Set<string>(); tasks.forEach((t) => t.assignees.forEach((a) => s.add(a)));
    return [...s].sort();
  }, [tasks]);
  ...
  function cycle(t: Task) {
    ...
    patch(t.id, (x) => ({ ...x, status: next })); if (persists) start(() => { setStatus(t.id, next); });
  }
  function addAssignee(t: Task, name: string) {
    if (!name || t.assignees.includes(name)) return;
    const pid = idByName.get(name); if (!pid) return;
    patch(t.id, (x) => ({ ...x, assignees: [...x.assignees, name].sort() })); if (persists) start(() => { toggleAssignee(t.id, pid, true); });
  }
  function removeAssignee(t: Task, name: string) {
    const pid = idByName.get(name); if (!pid) return;
    patch(t.id, (x) => ({ ...x, assignees: x.assignees.filter((a) => a !== name) })); if (persists) start(() => { toggleAssignee(t.id, pid, false); });
  }
  function move(t: Task, projectId: string) {
    patch(t.id, (x) => ({ ...x, project_id: projectId })); if (persists) start(() => { moveTask(t.id, projectId); });
  }
      ...
      {!showingCards && (
        <>
          ...
          <div className="controls">
            ...
            {view === "tasks" && (
              <select className="streamsel" value={assignee} onChange={(e) => setAssignee(e.target.value)} title="Filter by assignee">
                <option value="">Anyone assigned</option>
                {assignedNames.map((n) => <option key={n} value={n}>{n} ({personOpen.get(n) ?? 0})</option>)}
              </select>
            )}
              ...
              {rows.slice(0, 300).map((t) => (
                <div key={t.id} className={`tcard ${t.status === "done" ? "done" : ""}`}>
                  <div className="tcard-top">
                    ...
                    <select className="proj-pick" value={t.project_id} onChange={(e) => move(t, e.target.value)} title="Move to project">
                  ...
                  {t.source_url && (
                    <div className="tcard-foot">
                      <a className="src" href={t.source_url} target="_blank" rel="noreferrer">source</a>
          ...
          {rows.length > 300 && <p className="note">Showing first 300 of {rows.length} — filter or search to narrow.</p>}
        </>
      )}
    </>
  );
