...
export default function Workspace({
  ...
  const nameById = useMemo(
    () => new Map(data.people.map((p) => [p.id, p.name])), [data.people]
  );
  const idByName = useMemo(
    () => new Map(data.people.map((p) => [p.name, p.id])), [data.people]
  );
  const projName = useMemo(
    () => new Map(data.projects.map((p) => [p.id, p.name])), [data.projects]
  );
  ...
  const personOpen = useMemo(() => {
    ...
    tasks.forEach((t) => {
      ...
      t.assignees.forEach((a) => m.set(a, (m.get(a) ?? 0) + 1));
    });
    return m;
  }, [tasks]);
  ...
  function patch(id: string, fn: (t: Task) => Task) {
  ...
  const selLabel = selProj ? projName.get(selProj) : selPerson;

  return (
    <>
      <div className="seg">
        <button className={view === "projects" ? "on" : ""} onClick={() => { setView("projects"); setSelPerson(null); }}>Projects</button>
        <button className={view === "people" ? "on" : ""} onClick={() => { setView("people"); setSelProj(null); }}>People</button>
      </div>
      ...
      {view === "projects" && (
        <div className="cards">
          {data.projects.map((p) => {
            ...
            return (
              <div key={p.id} className={`card ${selProj === p.id ? "sel" : ""}`} onClick={() => setSelProj(selProj === p.id ? null : p.id)}>
      ...
      {view === "people" && (
        <div className="cards">
          {data.people.filter((p) => (personOpen.get(p.name) ?? 0) > 0 || p.name === primaryUser)
            ...
            .map((p) => (
              <div key={p.id} className={`card ${selPerson === p.name ? "sel" : ""}`} onClick={() => setSelPerson(selPerson === p.name ? null : p.name)}>
      ...
      <div className="controls">
        ...
        {selLabel && <button className="clear" onClick={() => { setSelProj(null); setSelPerson(null); }}>× {selLabel}</button>}
      ...
      {rows.length > 400 && <p className="note">Showing first 400 of {rows.length} — filter to narrow.</p>}
    </>
  );
}
