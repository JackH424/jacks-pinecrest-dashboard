...
export async function GET(req: Request) {
  ...
  const rows = (await sql`
    SELECT t.id AS task_id, t.title, t.due, p.id AS pid, p.name AS assignee
    FROM tasks2 t
    JOIN task_assignees ta ON ta.task_id = t.id
    JOIN people p ON p.id = ta.person_id
    WHERE t.status <> 'done' AND t.due <> '' AND t.due <= ${today}`) as
    { task_id: string; title: string; due: string; pid: string; assignee: string }[];
  ...
  for (const r of rows) {
    ...
    if ((res as unknown[]).length) posted++;
  }
  return Response.json({ ok: true, candidates: rows.length, reminders_posted: posted });
}
